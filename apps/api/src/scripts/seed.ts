...
async function seed() {
  ...
  // Default admin user
  const existingAdmin = await User.findOne({ email: 'admin@cholonbilorganic.com' });
  if (!existingAdmin) {
    await User.create({
      name: 'Super Admin',
      email: 'admin@cholonbilorganic.com',
      password: 'Admin@1234',
      role: adminRole._id,
      isActive: true,
    });
    console.info('✅ Created admin user: admin@cholonbilorganic.com / Admin@1234');

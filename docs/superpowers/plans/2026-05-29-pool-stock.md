...
```bash
cd /Users/afnanmahmud/Documents/sodaikini-organic
...
```bash
cd /Users/afnanmahmud/Documents/sodaikini-organic/apps/api
pnpm tsc --noEmit
```
...
In `apps/web/src/features/stock/stock.api.ts`, the `AddStockInput` and `AdjustStockInput` types now come from `@sodaikini/types` so no change is needed there — but verify the `StockMovementFilters` type — remove `variantId` if present:
...
```bash
cd /Users/afnanmahmud/Documents/sodaikini-organic/apps/web
pnpm tsc --noEmit
```

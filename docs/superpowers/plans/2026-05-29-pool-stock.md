...
```bash
cd /Users/afnanmahmud/Documents/cholonbil-organic
pnpm --filter @cholonbil/types build
```
...
```bash
cd /Users/afnanmahmud/Documents/cholonbil-organic/apps/api
pnpm tsc --noEmit
```
...
In `apps/web/src/features/stock/stock.api.ts`, the `AddStockInput` and `AdjustStockInput` types now come from `@cholonbil/types` so no change is needed there — but verify the `StockMovementFilters` type — remove `variantId` if present:
...
```bash
cd /Users/afnanmahmud/Documents/cholonbil-organic/apps/web
pnpm tsc --noEmit
```

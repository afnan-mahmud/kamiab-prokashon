...
cd /Users/afnanmahmud/Documents/kamiab-organic/apps/api
pnpm dev &
curl -s http://localhost:3091/api/admin/stock/summary -H "Authorization: Bearer <token>" | jq .
```
...
```bash
curl -s -X POST http://localhost:3091/api/admin/stock/movements \
...
```bash
curl -s http://localhost:3091/api/admin/products/<id> -H "Authorization: Bearer <token>" | jq '.data.poolStock'
```

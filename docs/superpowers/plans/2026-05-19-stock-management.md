...
```bash
pnpm --filter @sodaikini/types build
```
...
```bash
pnpm --filter @sodaikini/api type-check
```
...
router.post('/:id/return', requirePermission('orders.edit'), async (req, res, next) => {
  try {
    ...
    order.status = 'Returned' as import('@sodaikini/types').OrderStatus;
    order.statusHistory.push({
      status: 'Returned' as import('@sodaikini/types').OrderStatus,
...
Add `ProcessReturnInput` to the import from `@sodaikini/types`:

```ts
import type { Order, PaginatedResponse, ProcessReturnInput } from '@sodaikini/types';
```
...
```bash
pnpm --filter @sodaikini/web type-check
```
...
```bash
pnpm --filter @sodaikini/types build && pnpm --filter @sodaikini/api type-check && pnpm --filter @sodaikini/web type-check
```

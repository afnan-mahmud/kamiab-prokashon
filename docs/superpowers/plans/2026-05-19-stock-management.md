...
```bash
pnpm --filter @cholonbil/api type-check
```
...
router.post('/:id/return', requirePermission('orders.edit'), async (req, res, next) => {
  try {
    ...
    order.status = 'Returned' as import('@cholonbil/types').OrderStatus;
    order.statusHistory.push({
      status: 'Returned' as import('@cholonbil/types').OrderStatus,
...
Add `ProcessReturnInput` to the import from `@cholonbil/types`:
...
```bash
pnpm --filter @cholonbil/web type-check
```
...
```bash
pnpm --filter @cholonbil/types build && pnpm --filter @cholonbil/api type-check && pnpm --filter @cholonbil/web type-check
```
